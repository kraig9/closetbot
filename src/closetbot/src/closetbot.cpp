#include "../include/closetbot/closetbot.hpp"
...
#include "../include/closetbot/detect.hpp"
...
// Closetbot::Closetbot(std::string const& port, uint32_t baudrate)
//     : serial_port_{std::make_unique<serial::Serial>(
//           port, baudrate, serial::Timeout::simpleTimeout(kTimeout))} {}

// Closetbot::Closetbot(std::unique_ptr<serial::Serial> serial_port)
...
fp::Result<response_t> Closetbot::send(Command const& command) {
...
// fp::Result<std::array<double, 6>> Closetbot::get_radians() {
//   // auto const raw_angles = send(get_angles());
//   auto const raw_angles = {0,0,0,0,0,0};
//   if (!raw_angles) {
...
//       int2angle(raw_angles->at(4)) * M_PIClosetbot::Closetbot(){}
//   }
// }


// fp::Result<response_t> Closetbot::send_radians(
...
//   if (!maybe_port) {
//     return tl::make_unexpected(fp::NotFound("Port of Closetbot not found"));
//   }
...
//         fmt::format("Attempt to connect to Closetbot failed at port: {}",
//                     maybe_port.value())));
//   }
